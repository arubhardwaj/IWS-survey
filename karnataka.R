...
karnataka <- read_csv("iws_karnataka.csv") # import data in R
head(karnataka)

karnataka$sound_jati <- soundex(karnataka$b7_text)
karnataka$sound_jati %>% table()

karnataka %>% filter(sound_jati=="V251") %>% View()
...
# hindu
kar_hindu <- karnataka %>% filter(b4==1)
...
write_csv(kh,"hindukarnataka.csv")
...
# muslim
kar_muslim <- karnataka %>% filter(b4==2)
...
write_csv(kh,"muslimkarnataka.csv")



karnataka_new <- rbind(km,kh)

write_csv(karnataka_new,"karnataka_new.csv")



jati_dict <- karnataka %>% select(b7,b7_text)
jati_dict <- jati_dict %>% filter(b7 != -888) %>% filter(b7!=-999)

jati_dict <- unique(jati_dict)
write_csv(jati_dict,"karntaka_jati_dict.csv")
View(jati_dict)
...
jati_dict <- unique(jati_dict)
write_csv(jati_dict,"karnataka_missing_jati.csv")
View(jati_dict)
jati_dict
...
karnataka %>% filter(b7==-888) %>% count(b6_text)

karnataka  %>% count(b6_text)



karnataka_new <- read_csv("karnataka_new.csv")

karnataka_new %>% filter(new_code==-888) %>% count(b6_text)

karnataka_new  %>% count(b6_text)
